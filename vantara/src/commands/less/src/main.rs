...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for filename in &paths {
        let lines: Vec<String> = if let Ok(file) = File::open(filename) {
        ...
        loop {
            ...
            for (i, (line_idx, _subline)) in wrapped_lines.iter().enumerate() {
...
fn print_usage() {
    ...
    safe_println(format_args!("     n | --number    Show number lines"));
